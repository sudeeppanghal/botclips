...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/80 dark:bg-[#111827]/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/80 px-4 sm:px-8 py-3.5 flex items-center justify-between gap-4">
      ...
      {/* Right side controls: Theme Toggle, Balance pill, Notifications, User profile */}
      <div className="flex items-center gap-1.5 sm:gap-3">
        {/* Quick Mobile Wallet Balance Pill */}
        <Link
          href="/dashboard/wallet"
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-bold text-xs hover:bg-blue-100 transition-colors"
          title="Wallet Balance"
        >
          <Wallet className="w-3.5 h-3.5" />
          <span>{currencySymbol}{walletBalance.toFixed(0)}</span>
        </Link>
        ...
        <div className="relative">
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-72 sm:w-80 bg-white dark:bg-[#131b2e] border border-slate-100 dark:border-slate-800 rounded-2xl shadow-xl p-4 z-50">
        ...
        <div ref={profileRef} className="relative">
          <button
            ...
            className="flex items-center gap-2 p-1 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors cursor-pointer"
          >
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-slate-800 dark:bg-slate-700 text-white flex items-center justify-center font-bold text-xs sm:text-sm shadow-xs">
